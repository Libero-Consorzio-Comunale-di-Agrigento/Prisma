...
--changeset esasdelli:GDM_PROCEDURE_AG_CHANGE_TIPODOC_QUERY runOnChange:true stripComments:false

CREATE OR REPLACE PROCEDURE ag_change_tipodoc_query (
   p_nome_query         IN   VARCHAR2,
...
IS
   id_documento_query   NUMBER;
   ret                  VARCHAR2 (32000);
/******************************************************************************
   NAME:       ag_change_tipodoc_query
   PURPOSE:    Cambiare l'area del modello cui la query con nome P_NOME e' associata.
                Per identificare la query, cercando di non spostare query di altri
   ...
   p_nome_cartella         IN   VARCHAR2 NOME DELLA query DA SPOSTARE
   p_nome_tipo_documento   IN   VARCHAR2 NOME TIPO DOCUMENTO DELLA query DA SPOSTARE
   p_nuova_area                 VARCHAR2 AREA IN CUI SPOSTARE LA CARTELLA
   p_area_cartella_padre   IN   VARCHAR2 AREA DELLA CARTELLA IN CUI LA query DA SPSOTARE e' CONTENUTA
   p_cm_cartella_padre     IN   VARCHAR2    CODICE MODELLO DELLA CARTELLA IN CUI LA query DA SPSOTARE e' CONTENUTA
...
BEGIN
   SELECT d_diz.id_documento
    ...
    WHERE q_diz.nome = p_nome_query
      AND q_diz.id_documento_profilo = d_diz.id_documento
      AND d_diz.id_tipodoc = td_diz.id_tipodoc
      AND td_diz.nome = p_nome_tipo_documento
	  AND TD_DIZ.AREA_MODELLO<>p_nuova_area
      AND l_diz.id_oggetto = q_diz.id_query
      AND l_diz.tipo_oggetto = 'Q'
   ...
   ret := f_sposta_documento (id_documento_query, p_nuova_area);
